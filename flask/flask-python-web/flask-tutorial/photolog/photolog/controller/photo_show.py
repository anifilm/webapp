...
def __get_download_info(photolog_id, prefix_filename=""):
    ...
    return send_from_directory(
        download_folder, download_filename, as_attachment=True, mimetype="image/jpg"
    )
...
def show_all(page=1):
    ...
    return render_template(
        "list.html", pagination=pagination, photos=photo_pages, sizeof_fmt=sizeof_fmt
    )
...
def search_photo():
    ...
    photos = (
        ...
        .filter(
            or_(
                Photo.comment.like("%" + search_word + "%"),
