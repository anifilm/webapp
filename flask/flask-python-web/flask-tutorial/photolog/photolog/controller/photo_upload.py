...
def remove(photolog_id):
    ...
    try:
        ...
        dao.rollback()
        Log.error(
            "Photo remove error => " + photolog_id + ":" + user_id + ", " + str(e)
        )
        raise e
...
class PhotoUploadForm(Form):
    ...
    tag = TextField(
        "Tag", [validators.Length(min=1, max=400, message="400자리 이하로 입력하세요.")]
    )
    comment = TextAreaField(
        "Comment", [validators.Length(min=1, max=400, message="400자리 이하로 입력하세요.")]
    )
