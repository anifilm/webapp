...
def __send_twit(twitter, photolog_id):
    ...
    try:
        ...
        twitter.update_status(status=photo_comment, media_ids=[response["media_id"]])
...
def __oauth(photolog_id):
    ...
    try:
        twitter = Twython(
            current_app.config["TWIT_APP_KEY"], current_app.config["TWIT_APP_SECRET"]
        )

        callback_svr = current_app.config["TWIT_CALLBACK_SERVER"]

        auth = twitter.get_authentication_tokens(
            callback_url=callback_svr + url_for(".callback", photolog_id=photolog_id)
        )
...
def callback(photolog_id):
    ...
    try:
        ...
        twitter = Twython(
            current_app.config["TWIT_APP_KEY"],
