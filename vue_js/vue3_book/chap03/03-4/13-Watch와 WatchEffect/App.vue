<template>
  <p>{{ count_o }}</p>
  <button v-on:click="count_o++">Option API 카운트 증가</button>
  <p>{{ count_c_1 }}</p>
  <button v-on:click="count_c_1++">Composition API 1st 카운트 증가</button>
  <p>{{ count_c_2 }}</p>
  <button v-on:click="count_c_2++">Composition API 2nd 카운트 증가</button>
  <p>상태: {{ state }}</p>
  <button v-on:click="onStop()">watchEffect 중지</button>
</template>
